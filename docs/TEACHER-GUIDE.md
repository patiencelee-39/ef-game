...
**文件版本**：v1.3（2026/04/14） — 加入 WM v5.0 規則說明 🔄v5.0  
...
| 逆向記憶機率     | `REVERSE_PROBABILITY`      | 0.5（50%）    | 0～1        | ⚠️ 僅限混合規則 |
...
記憶關（帶「·記憶關」的場地）的方向已自動綁定到規則類型，無法更改：

| 場地編號 | 遊戲場地 | 規則類型 | WM 記憶方向 |
|---------|---------|---------|-----------|
| B / H | 規則一 + 記憶 | 建立基礎規則 | 順向 👉 |
| D / J | 規則二 + 記憶 | 反轉規則 | 逆向 🔄 |
| F / L | 混合 + 記憶 | 認知彈性 | 隨機 🎲 |

> **小提示**：如果想調整 WM 難度，改 `HIGHLIGHT_DURATION_MS`（位置亮起時間）或 `MIN_POSITIONS` / `MAX_POSITIONS`（位置數量），不要試著改 `REVERSE_PROBABILITY` — 它只對混合規則有效！
...
WM 共有 4 個可調參數，但要注意**方向是固定的**，由規則類型決定：

| 參數 | 現有值 | 用途 | 是否可改 |
|------|-------|------|--------|
| `MIN_POSITIONS` | 2 | 最少記憶位置數 | ✅ 可改 |
| `MAX_POSITIONS` | 6 | 最多記憶位置數 | ✅ 可改 |
| `HIGHLIGHT_DURATION_MS` | 800 | 位置亮起時間（毫秒） | ✅ 可改 |
| `REVERSE_PROBABILITY` | 0.5 | 混合規則中逆向的機率 | ✅ 可改（⚠️ 僅限混合） |

**🔒 WM 方向已固定** — 不能改：
- **規則一 + WM** → 必定「順向」（Forward）
- **規則二 + WM** → 必定「逆向」（Reverse）
- **混合 + WM** → 隨機決定（50%順向 / 50%逆向）

> **為什麼？**因為新規則（v5.0）將 WM 方向與規則類型綁定，以強化認知彈性訓練。規則二本身是「反轉規則」，所以 WM 也自動反轉；規則一是基礎規則，所以 WM 也是基礎順向；混合規則才保留隨機性。

**修改範例**：讓 WM 更簡單

**需求**：幼兒記不住，想讓位置亮起的時間更長。

**步驟**：
1. 搜尋 `HIGHLIGHT_DURATION_MS`
2. 把 `800` 改成 `1200`（位置亮起時間從 0.8 秒改成 1.2 秒）：
   ```
   HIGHLIGHT_DURATION_MS: 1200, // ← 教師可改：位置亮起時間（1.2秒）
   ```
