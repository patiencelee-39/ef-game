...
│   └── result.html                # 成績結果
├── singleplayer/
│   ├── adventure-map.html         # 探險地圖頁面 ⭐ Phase 4
│   ├── free-select.html           # 自由選擇頁面 ⭐ Phase 4
│   ├── game.html                  # 遊戲進行（模組化重寫）⭐ Phase 4
│   └── result.html                # 結算頁面（雙模式 + 分析報告）⭐ Phase 4
├── leaderboard/
│   ├── class.html                 # 班級排行榜（+ CSV 報告）
│   ├── live.html                  # 即時排行看板（+ 報告雙面板） ⭐ v2.2
│   └── global.html                # 全球排行榜
├── management/
...
│   ├── adventure-maps-config.js   # 探險地圖配置 ⭐ Phase 4
│   ├── svg-assets.js              # SVG 刺激物資源 ⭐ Phase 4
│   ├── game-logic.js              # 遊戲邏輯（多人模式）
│   ├── utils/
│   │   ├── room-code.js          # 房間代碼處理
│   │   ├── bonus-calculator.js   # 加權計算
│   │   ├── storage.js            # localStorage 封裝
│   │   ├── score-calculator.js   # 規則分數 + WM 分數 + 星星計算 ⭐
│   │   ├── level-calculator.js   # 等級系統ﾈ5 階） ⭐ Phase 4
│   │   ├── badge-checker.js      # 徽章系統ﾈ18 枚） ⭐ Phase 4
│   │   ├── export.js             # 截圖 & CSV 匯出
...
│   ├── singleplayer/
│   │   ├── progress-tracker.js   # 進度追蹤器 ⭐ Phase 4
│   │   └── mode-controller.js    # 模式控制器 ⭐ Phase 4
│   ├── stages/
│   │   ├── stage-A.js            # 場地 A 邏輯
│   │   ├── stage-B.js
│   │   ├── stage-C.js
...
│   │   ├── bonus-display.css     # 加權獎勵顯示
│   │   ├── toggle-switch.css     # 開關按鈕
│   │   └── csv-report.css        # 分析報告樣式（BEM + print + dark） ⭐ v2.2
│   └── themes/
│       └── stages.css            # 各場地主題
│
├── audio/（參考聲音設計文件）
...
總確認項目：60+
核心修正項目：6個 ⭐
已完成 Phase：0, 1, 3, 4（Phase 2 部分完成）
JS 模組數量：~22 個（含 csv-report.js, constants.js 擴充）
HTML 頁面數量：~16 個（含多人/單人/管理/排行 + live.html）
單人模式新增：8 + 3 個檔案（原 Phase 4 + csv-report 相關）
CSS 設計 Token：30+ 變數
CSS 元件樣式：csv-report.css（BEM + print + dark）
CDN 依賴：Chart.js 4.4.1 / PapaParse 5.4.1 / html2pdf.js 0.10.1
Firebase 服務：Hosting + Realtime Database
...
**文件完成日期**：2026年2月8日（初版）/ 2026年2月14日（v2.1 更新）  
**文件版本**：v2.1  
**文件作者**：小摩研究助理
...
index.html
  ...
  │                                                                   └─ 🗺️ 回到地圖 → adventure-map.html
  │
  ├─ 多人模式 → multiplayer/room-join.html
  │                  └─ waiting-room.html → game.html → result.html
  │
  └─ 排行榜 / 報告
       ├─ leaderboard/class.html     班級排行榜（CSV 上傳 + 報告 + PDF + 截圖）
       └─ leaderboard/live.html      即時排行看板（Firebase 即時 + 報告雙面板）
```
