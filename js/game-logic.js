...
const GameLogic = {
  ...
  showStimulus: function () {
    ...
    // === 第四回合：釣魚 + night 條件 ===
      ...
      if (this.state.isNightTime) {
        ...
        if (CONFIG.DEBUG_MODE) {
          console.log(`  🌛 晚上 | 刺激物: ${stimulus} | 正確答案：不按`);
        }
      } else {
        this.elements.backgroundLayer.innerHTML = SVG_ASSETS.oceanBg;
        this.elements.personIndicator.style.display = "none";
        ...
        if (CONFIG.DEBUG_MODE) {
          ...
          console.log(`  ☀️ 白天 | 刺激物: ${stimulus} | 正確答案：${answer}`);
        }
      }
  ...
  showScreen: function (screenName) {
    ...
    if (this.elements.screens[screenName]) {
      ...
      console.error(`❌ 找不到畫面: ${screenName}`);
    }
  },
  ...
  exportData: function () {
    ...
    this.state.results.forEach((result) => {
      ...
      row[F.HAS_PERSON || "HasPerson"] = result.hasPerson;
      row[F.IS_NIGHT_TIME || "IsNightTime"] = result.isNightTime;
