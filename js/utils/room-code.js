...
// 引入常數
const { ROOM_CODE_LENGTH, VALID_ROOM_CODE_CHARS, MAX_ROOM_CODE_GENERATION_ATTEMPTS } =
 ...
 * 排除：I, O, 0, 1
 * 使用：A-Z（除I,O）+ 2-9
 * @returns {string} 5 位字元的房間代碼
 */
function generateRoomCode() {
  let code = '';

  for (let i = 0; i < ROOM_CODE_LENGTH; i++) {
    const randomIndex = Math.floor(Math.random() * VALID_ROOM_CODE_CHARS.length);
...
function isValidRoomCode(code) {
  ...
  const validChars = new RegExp(`^[${VALID_ROOM_CODE_CHARS}]{${ROOM_CODE_LENGTH}}$`);
...
/**
 * 生成唯一的房間代碼
 * @param {number} [maxAttempts=MAX_ROOM_CODE_GENERATION_ATTEMPTS] - 最大嘗試次數
 * @returns {Promise<string>} 唯一的房間代碼
 * @throws {Error} 當無法生成唯一代碼時拋出錯誤
 */
async function generateUniqueRoomCode(maxAttempts = MAX_ROOM_CODE_GENERATION_ATTEMPTS) {
