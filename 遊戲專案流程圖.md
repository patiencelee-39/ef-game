> ⚠️ **已棄用（Deprecated）**  
> 本文件內容已全數嵌入 [完整需求文件 v4.5](完整需求文件v4.5.md) 各對應章節。  
> 此檔案僅作為歷史備份保留，**請勿再修改本文件**。  
> 棄用日期：2026/02/13
...
> **對應文件**：[完整需求文件 v4.4](完整需求文件v4.4.md)（2026/02/13）  
> **流程圖語法**：Mermaid  
> **總數**：30 個流程圖 + 3 個開發路線圖，分 6 大類  
> **v4.3 變更**：探險地圖取代遊戲場+標準模式、自由選擇取代自由模式、WM≥83%通過、全員完成廣播+觀看總結果按鈕鎖定解鎖、房主變更通知區分新房主/其他玩家  
> **v4.4 變更**：§1.3 流程圖索引完整交叉連結（30 流程圖↔需求章節雙向導航）  
> **v4.4+ 變更**：新增開發路線圖章節（Phase 依賴圖 + 甘特圖 + 決策時間線）
